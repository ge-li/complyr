...
ivpim <- function(y,
  ...
  if ("gam" %in% class(ps_model)) {
    ps_model_matrix <- mgcv::model.matrix.gam(ps_model)
  } else if ("lm" %in% class(ps_model)) {
    ps_model_matrix <- stats::model.matrix(ps_model)
  }
  drift_right <- kvs * ps_model_matrix[rep(ind, n), , drop = F] + # (j: 12...n, 12...n, ...)
                 vsk * ps_model_matrix[rep(ind, rep(n, n)), , drop = F] # (i: 1...1, 2...2, ...)
